...
export default function AddModal() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(`${BackendUrl}/todo/create-todo`, 
  ...
  return (
    <Modal
      ...
      >
        <Box sx={style}>
        <Box className={styles.contactTitle}>Add Todo</Box>
        <form onSubmit={handleSubmit}>
          <TextField
            error={isError.isTitleError}
            id="standard-error-helper-text"
            label="Title"
            helperText={isError.isTitleError && "title must be at least 6 characters long "}
            variant="standard"
